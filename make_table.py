...
class MakeTable():
    ...
    async def generate_pair_time_table(self, input_table):
        ...
        pair_time_table = pd.concat(
            (pair_time_table, pair_time_table), axis=0).sort_index().reset_index(drop=True)
    ...
    async def generate_full_table(self, input_table, target_group):
        ...
        days_of_the_week = pd.Series(np.array(
        ...
        full_table = pd.concat(
            (
                days_of_the_week,
                pd.concat(
                    [pair_time_table] * (partial_input_table.shape[0] // pair_time_table.shape[0]), axis=0
        ...
        for day_of_the_week, table_daily in table.groupby(('День недели')):
            ...
            for pair_number, table_pairly in table_daily.groupby(('Номер пары')):
                ...
                if table_pairly.iloc[0]['Предмет'] != '':
                    for el in list(
                        table_pairly[['Предмет', 'Вид занятий', 'Преподаватель', 'Аудитория']].iloc[0].values):
    ...
    async def return_processed_table_data(self, tomorrow_bias=0):
        ...
        table_data, table_data_downloaded, table_modified_date = (await self.get_table()).values()

        table_data = table_data[self.week_type]

        table_data = {days_of_the_week[str(key)]:value for key, value in table_data.items()}
        ...
        for key, values in table_data.items():
            ...
            if sum([len(el) for el in values.values()]):
                ...
                for pair_number, value in values.items():
                    if value:
                        new_table_data[key].update({pair_number:value})  #new_table_data[key][pair_number] = value
    ...
    async def make_week_table(self):
        ...
        table_data, table_data_downloaded, table_modified_date = await self.return_processed_table_data(tomorrow_bias=1)

        current_day = days_of_the_week[str(datetime.now().weekday() + 1)]

        table_str = f'ПОЛУЧЕНО ИЗ КЭША ({str(table_modified_date)[:16]})\n\n' if not table_data_downloaded else ''
        table_str += self.target_group + "\nСегодня " + \
            '.'.join((str(el) for el in list(datetime.now().date().timetuple())[:3][::-1])) + \
            " (" + current_day + ")\n"


        table_str += \
            self.week_type[0].upper() + self.week_type[1:] + \
            ' неделя (' + str(self.week_delta) + ')\n\n'
    ...
    async def make_today_table(self):
        ...
        table_data, table_data_downloaded, table_modified_date = await self.return_processed_table_data()
    ...
    async def make_timetable_image(self, 
        timetable_text, 
        font_path,
        color=(255, 255, 255),
        text_hight=15,
